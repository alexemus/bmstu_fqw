...
def model():
    ...
    if request.method == 'POST':
        ...
        if error == '':
            ...
            preprocessor_mod = load_pickle_obj('preprocessor_mod')
            model = load_pickle_obj('model_best')
            x = preprocessor_mod.transform(x)
            y = model.predict(x)
            var8 = y[0]
    # Отображение результата
    return render_template('model.html', params=params, error=error, inputs=x.to_html(), var8=var8, var9=var9)
